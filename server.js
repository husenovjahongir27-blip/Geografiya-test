...
// index.html asosiy papkada joylashgani uchun
...
app.get("/api/telegram-test", async (req, res) => {
  try {
    ...
    const sent = await telegram("sendMessage", {
      chat_id: CHAT,
      text:
        "✅ Maktab Test: Telegram ulanishi muvaffaqiyatli ishlayapti."
    });
...
// Test natijasini Telegramga yuborish
app.post("/api/result", async (req, res) => {
  try {
    const x = req.body;

    if (!x.student || !x.details) {
      return res.status(400).json({
        ok: false,
        error: "Natija ma’lumotlari to‘liq emas."
      });
    }
    ...
    if (x.time) {
      m += `⏱️ Vaqt: ${x.time} daqiqa\n`;
    }

    m += `🔢 Jami: ${x.total} ta\n`;
    m += `✅ To‘g‘ri: ${x.correct} ta\n`;
    m += `❌ Xato: ${x.wrong} ta\n`;
    m += `📊 Foiz: ${x.percent}%\n`;
    m += `🏆 Baho: ${x.grade}\n\n`;
    ...
    for (const d of x.details) {
      ...
      if (!d.isCorrect) {
        m += ` → To‘g‘ri javob: ${clean(d.correctAnswer)}`;
      }

      m += "\n";
    }

    const j = await telegram("sendMessage", {
      chat_id: CHAT,
      text: m
    });
    ...
    if (!j.ok) {
      ...
      return res.status(502).json({
        ok: false,
        error: j.description || "Telegram xatosi",
        step: "sendMessage",
        http: j.http
      });
    }
...
app.listen(PORT, async () => {
  ...
  try {
    ...
    if (me.ok) {
      ...
      console.error(
        `Telegram token xatosi: ${me.description}`
      );
    }

  } catch (e) {
    console.error(
      `Telegramga ulanish xatosi: ${e.message}`
    );
  }
});
